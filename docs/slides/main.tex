...
%--------------------------------------------------------------------- 3 ----
\begin{frame}{Dataset and task}
  % NOTE: a frame body must not begin with a bare {...} group. With the custom
  % frametitle template above, beamer absorbs it and the content silently
  % disappears from the PDF with no error. Start with a command instead.
  \small\textbf{Dataset:} the \hl{UMUD} benchmark (Ritsche et al., 2026), which
  indexes the DL\_Track\_US training sets.
  \textcolor{Muted}{1\,048 aponeurosis and 2\,761 fascicle frames, 309 unlabelled
  test frames, six scanner families.}\normalsize
  \vspace{0.1cm}

  It does not make this a supervised regression problem.
  \vspace{0.18cm}

  \begin{columns}[T]
  \begin{column}{0.48\textwidth}
    \textbf{\color{Red} No measurement labels}\\[0.15cm]
    {\footnotesize Training data gives \emph{masks} only, never a thickness or an angle.
    The three targets must be \hl{reconstructed geometrically}.}
    \vspace{0.22cm}

    \textbf{\color{Red} No scale metadata}\\[0.15cm]
    {\footnotesize Two targets are in millimetres; the network sees pixels. The
  ...
  \begin{column}{0.48\textwidth}
    ...
    \begin{itemize}\setlength\itemsep{0.15cm}
      ...
      \item 26\% of fascicle images are duplicates, so a naive split leaks
            \num{179} groups into validation. Content-grouped splitting
            reduces that to \num{0}
    \end{itemize}
    Neither defect raises an error.}
  \end{column}
  \end{columns}
