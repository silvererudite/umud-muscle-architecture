...
\paragraph{Significance.} A still image whose texture is glued to the grid may
...
metric at all. The weakness (developed in Section~\ref{sec:limits}) is that the
...
\subsection{Continuous-signal formulation}
...
\subsection{Equivariance metrics}
...
subsequent ablation legible, though Section~\ref{sec:limits} argues it also creates a
...
\paragraph{Strengths.} The work is \emph{theory-first} in a literature that
...
\paragraph{Theory versus implementation.} The continuous-domain
...
\paragraph{Treatment of the discriminator.} Only the
...
\subsection{Ablation across configurations}
...
\begin{table}[htbp]
\centering
\caption{Training cost on FFHQ $1024^2$ (paper, Section~4).}
\label{tab:cost}
\small
\begin{tabular}{lrrr}
\toprule
...
\subsection{Limitations acknowledged by the authors}
...
\subsection{Additional gaps}

\paragraph{G1. Self-validating metrics.} EQ-T and EQ-R are introduced by
...
\paragraph{G2. FID parity is a non-regression.}
...
\paragraph{G3. Global equivariance versus local motion.} The opening example
...
\paragraph{G4. Equivariance assumed universally desirable.} The paper
...
\paragraph{G5. Scientific and medical imaging.} This gap follows
...
\paragraph{G6. Limited domain coverage.} Five of
...
%======================================================== 5. CONCLUDING  =====
\section{Concluding Remarks and Suggested Improvements}
...
suggestions below are aimed at the gaps of Section~\ref{sec:limits}, ordered from
cheapest to most speculative.

\subsection*{S1. Equivariant discriminator}
...
\subsection*{S2. Independent validation of equivariance}
...
\subsection*{S3. Adaptive band-limiting}
...
\subsection*{S4. Local deformation equivariance}
...
warp's Jacobian. This would let one state and test the paper's motivating claim (that a nose moves with a head) rather than a proxy for it. Spatial
transformer machinery~\citep{jaderberg2015stn} offers a ready parameterisation.

\subsection*{S5. Selective equivariance}
...
\subsection*{S6. Layer-selective rotation equivariance}
