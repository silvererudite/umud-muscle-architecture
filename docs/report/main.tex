...
% Works both in the repository (figures live in ../figures/) and in the
% Overleaf bundle (figures/ sits beside main.tex).
...
pennation angle to \textbf{\ang{1.16}} relative to the annotator's own masks,
and aponeurosis segmentation reaches a Dice of \textbf{0.839}.
...
($p=0.0003$), and what it actually contributes is boundedness against
$1/\sin$ ill-conditioning at shallow pennation angles, a property the original
reasoning never identified.
\end{abstract}
...
The UMUD benchmark~\citep{umudchallenge2026} provides 1\,048 aponeurosis-annotated
and 2\,761 fascicle-annotated training frames, together with 309 unlabelled test
frames for which all three parameters must be produced.
...
\begin{enumerate}
  \item A complete, reproducible pipeline (ruler calibration, segmentation
        and geometric reconstruction) that produces all three architecture
        parameters from a single ultrasound frame without any manual input.
  ...
  \item A \textbf{pseudo-ground-truth evaluation protocol} that makes offline
        model selection possible in a task that provides no measurement labels
        of any kind.
...
\subsection{Image and mask size mismatch}
...
\paragraph{Coverage.} \textbf{100\,\% of the 309 test frames calibrate.} The
...
\paragraph{Fallback behaviour.} An early version of the generic fallback
...
\paragraph{Validation of the orientation targets.} The supervision is derived from the
...
The data provides no measurement labels, so no direct error against a human
measurement can be computed for any frame. We therefore use a
\textbf{pseudo-ground-truth} protocol: run the \emph{same} geometric reconstruction twice, once on the
...
\paragraph{External consistency check.} Pennation angle is scale-invariant, so running the
...
for vastus lateralis, and \ang{16.4} for an independently developed pipeline
applied to the same frames~\citep{dread2026vera}. Two independent
...
Orientation-field error & n/a & \textbf{\ang{2.74}} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Interpretation of the fascicle Dice.} A Dice of 0.27 is low, and the
...
\paragraph{Scope of the paired evaluation.} All 78 paired frames are $512\times512$ crops from a
...
with no per-family ground truth the two cannot be separated from the data
available here. This is the weakest link in the generalisation story and the most
likely source of the pipeline's residual error.
...
\subsubsection{Fascicle length construction}
\label{sec:abl-fl}

\begin{table}[htbp]
\centering
\caption{Fascicle-length construction, evaluated on the 78 paired frames against
the annotator's masks through the same geometry module.}
\label{tab:abl-fl}
\begin{tabular}{lrrr}
\toprule
\textbf{Construction} & \textbf{Median rel.\ error} & \textbf{Mean} & \textbf{Frames won} \\
\midrule
Intersection \eqref{eq:intersect} & 0.077 & 0.162 & 32\,\% \\
Textbook $\MT/\sin\PA$ \eqref{eq:textbook} & \textbf{0.060} & \textbf{0.128} & \textbf{68\,\%} \\
\bottomrule
\end{tabular}
\end{table}

The textbook formula wins decisively on this set: better on 68\,\% of frames,
Wilcoxon signed-rank $p=0.0003$. This contradicts the premise of the original
proposal, which argued that the intersection should be preferred because the
aponeuroses are not parallel.

The intersection does not lose because the divergence is estimated badly; it
is recovered to \ang{0.15}. It loses because of what each formula depends on. The
textbook formula leans on muscle thickness, which the pipeline recovers to
...
over. A model that is more faithful in principle produced a worse estimate in
practice by depending on noisier inputs.

\paragraph{Where each construction fails.} The paired set does not exercise both
constructions equally, and the difference is systematic.

\begin{table}[htbp]
\centering
\caption{Pennation-angle distributions. $\MT/\sin\PA$ diverges as $1/\sin$, so
frames at shallow pennation dominate its error.}
\label{tab:pa-dist}
\begin{tabular}{lrr}
\toprule
...
At \ang{8}, \eqref{eq:textbook} predicts a \SI{167}{\milli\metre} fascicle, which
is outside any physiological range. The paired set contains almost none of that
regime (3\,\% of frames below \ang{12}, against 15\,\% on the test frames),
so the divergence never bit there, and the textbook formula's better use of the
well-recovered thickness won on the merits. The intersection cannot diverge,
because the fascicle ray meets the superficial aponeurosis at a finite distance
however shallow the pennation angle.

The two constructions therefore fail in different regimes, and the pipeline uses
each where it is better conditioned: \texttt{fl\_method="auto"} takes the
textbook value while it lands inside the physiological range and the intersection
when it does not. This introduces no tuned parameter: the bound it tests
against is the clamp the estimate would have hit anyway, and it is a no-op
when the aponeuroses are parallel, because the two constructions then coincide
exactly. On the test frames it engaged on 41 of 309.

It should be stated plainly that this rule is justified by the mechanism and by
the pennation-angle statistics of Table~\ref{tab:pa-dist}, \emph{not} by a
measured error on the test frames, since those frames carry no labels. The
paired-set comparison in Table~\ref{tab:abl-fl} covers only the regime in which
the textbook formula is well conditioned.

\paragraph{Relation to the original hypothesis.} The proposal argued for the
intersection from aponeurosis non-parallelism, and on that account it loses
outright. What it contributes instead is boundedness against $1/\sin$
ill-conditioning, a different property, which the original reasoning did not
identify.

\subsubsection{Orientation head}
...
\subsubsection{Per-frame confidence}
...
\begin{enumerate}
  ...
  \item \textbf{The paired set is small and homogeneous.} All 78 frames come from
        one scanner family, and Section~\ref{sec:abl-fl} shows it covers only part
        of the pennation-angle range present in the test frames. It is the only
        end-to-end evidence available, but it is not a representative sample.
...
Section~\ref{sec:generalisation}, which is where the pipeline's residual error
most plausibly lies.

\section{Conclusion}

We built an automatic muscle-architecture estimator that recovers muscle
thickness to 0.3\,\% and pennation angle to \ang{1.16} against annotator masks,
with aponeurosis segmentation at a Dice of 0.839 and fascicle orientation to
\ang{2.74}.
